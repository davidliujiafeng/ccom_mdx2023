...
* Unzip the label-noise dataset into `./demucs/dataset/label_noise/train/`. Put the 3 training config files from this repo (`./conf/stepX.yaml`) into `./demucs/conf/variant/`.The dir looks like this:
```
demucs
|-+conf
|---+variant
...
* Re-write the loss function part in `./demucs/demucs/solver.py` (line 324) as above.
